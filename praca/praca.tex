...
\begin{enumerate}
  ...
  \item Urochomić symulacje przeprowadzane za pomocą systemu
...
z plików w formacie CSV oraz położenia MOP-ów z pliku w formacie xml. Pliki te muszą spełniać pewne
...
które w Mopniku są ustawione jako parametry domyślne, które mogą być zmienione
z poziomu użytkownika.
\subsubsection{Wyświetlanie wyników metodyk}
Liczby miejsca parkingowych są obliczane dla konkretnych odcinków dróg.
...
\begin{itemize}
  ...
  \item Liczba istniejących miejsca parkingowych (suma liczb miejsc
    parkingowych na MOP-ach znajdujących się na tym odcinku).
\end{itemize}
Informacje te pozwalają na ustalienie stref priorytetowych: odcinków dróg, na
których liczba miejsc parkingowych nie jest wystarczająca. Realizowane jest to
przy uruchomieniu programu -- wczytywane są domyślne dane i wyniki metodyk są
obliczane na podstawie domyślnych parametrów. Przy wczytaniu nowych danych,
dodaniu MOP-a, lub zmianie parametrów metodyki następuje przeładowanie mapy. 

\subsubsection{Dodatkwowe metodyki} 
Istniejąca implementacja pozwala na dodanie nowych metodyk z poziomu
programisty tak, aby użytkownik mógł mieć wybór.
...
\bibitem[9] {metodyki} M. Spławińska, K. Solecka, \\
